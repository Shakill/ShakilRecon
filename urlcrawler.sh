...
cat /root/recon/$domain/subdomain/good/final/best/all_active_sub.txt  | gauplus -t 30 | grep $domain | tee -a /root/recon/$domain/url/gaplus-urls.txt
...
cat /root/recon/$domain/url/url_httpx.txt | grep $domain | sort -u | tee -a /root/recon/$domain/url/valid_urls.txt
